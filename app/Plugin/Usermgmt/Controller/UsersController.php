...
class UsersController extends UserMgmtAppController
{
    ...
    public function myprofile()
    {
        ...
        //TODO refacor for optimization
        $mycourses = $this->CourseUser->find('all',array('conditions'=>array('User.id =' => $userId )));
        ...
        if ($this->request->isPost()) {
            ...
            if ($this->User->LoginValidate()) {
                ...
                if ($user['User']['password'] === $hashed) {
                    ...
                    $courseId = $this->Session->read('SelectedCourse.id');
                    if(!empty($courseId)){
                        $this->redirect(array('plugin' => '', 'controller' => 'courses', 'action' => 'join_course', $courseId));
                    }
//
//                    debug($OriginAfterLogin);
//                    debug(loginRedirectUrl);
//                    debug($redirect);
//                    die();
//
                    //debug(loginRedirectUrl );
                    // die();
                    //$this->redirect($redirect);//this is prev
    ...
    public function changeEmail()
    {
        ...
        if ($this->request->isPost()) {
            if ($this->User->save($this->request->data)) {
                $this->Session->setFlash(__('Email changed successfully'));
                $this->redirect('/setting');
            }
        }
