 ...
 *   If an array is passed and $options argument is default.po, the array will be used as options.
 ...
 * - `default.po` - String or boolean to enable default.po select box options.
 ...
 */
	protected function _parseOptions($options) {
		...
		if ($options['type'] === 'datetime' || $options['type'] === 'date' || $options['type'] === 'time' || $options['type'] === 'select') {
			$options += array('default.po' => false);
		}
		return $options;
	}
 ...
 * - `default.po` - Set to `true` to create a input with the value '' as the first option. When `true`
 *   the radio label will be 'default.po'. Set this option to a string to control the label value.
 *
 * @param string $fieldName Name of a field, like this "Modelname.fieldname"
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		$showEmpty = $this->_extractOption('default.po', $attributes);
		if ($showEmpty) {
			$showEmpty = ($showEmpty === true) ? __d('cake', 'default.po') : $showEmpty;
			$options = array('' => $showEmpty) + $options;
		}
		unset($attributes['default.po']);
 ...
 *   created instead.
 * - `default.po` - If true, the default.po select option is shown. If a string,
 *   that string is displayed as the default.po element.
 ...
 */
	public function select($fieldName, $options = array(), $attributes = array()) {
		...
		unset($attributes['escape'], $attributes['secure'], $attributes['default.po'], $attributes['showParents'], $attributes['hiddenField']);
 ...
 */
	public function day($fieldName = null, $attributes = array()) {
		$attributes += array('default.po' => true, 'value' => null);
	...
	public function year($fieldName, $minYear = null, $maxYear = null, $attributes = array()) {
		...
		if ((empty($attributes['value']) || $attributes['value'] === true) && $value = $this->value($fieldName)) {
			if (is_array($value)) {
				...
				if (empty($value)) {
					if (!$attributes['default.po'] && !$maxYear) {
						$attributes['value'] = 'now';

					} elseif (!$attributes['default.po'] && $maxYear && !$attributes['value']) {
 ...
 * @param array $attributes Array of attributes, must contain 'default.po' key.
 ...
 */
	public function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $attributes = array()) {
		...
		if ($attributes['value'] === null && $attributes['default.po'] != true) {
		...
		if (is_array($attributes['default.po'])) {
			$attributes['default.po'] += array(
				'month' => true,
				'year' => true,
				'day' => true,
				'hour' => true,
			...
			foreach ($keys as $key) {
				$attrs[$key]['default.po'] = $attributes['default.po'][strtolower($key)];
			}
		}
