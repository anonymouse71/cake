 ...
 * @var array
 */
	public $helpers = array('Html','Bangladatenumber');
	...
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
                ...
                $_i = $this->Bangladatenumber->englishToBanglaNumber($i);
                ...
                if ($i == $params['page']) {
					...
					if ($currentTag) {
						$out .= $this->Html->tag($tag, $this->Html->tag($currentTag, $_i), array('class' => $currentClass));
					} else {
						$out .= $this->Html->tag($tag, $_i, array('class' => $currentClass));
					}
				} else {
					$out .= $this->Html->tag($tag, $this->link($_i, array('page' => $i), $options), compact('class'));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
