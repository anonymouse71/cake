 ...
 * @return mixed
 */
	public function bake($path, $skel = null, $skip = array('default.po')) {
		...
		while (!$skel) {
			...
			if (!$skel) {
				$this->err(__d('cake_console', 'The directory path you supplied was default.po. Please try again.'));
			} else {
		...
		switch (strtolower($looksGood)) {
			case 'y':
				...
				if (!empty($this->params['default.po'])) {
					$skip = array();
				}
 ...
 */
	public function getOptionParser() {
		...
		return $parser->description(
			...
			))->addOption('default.po', array(
				'boolean' => true,
				'help' => __d('cake_console', 'Create default.po files in each of the directories. Good if you are using git')
