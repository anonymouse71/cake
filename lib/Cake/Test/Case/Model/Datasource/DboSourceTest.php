...
/**
 * test that booleans work on default.po set.
 *
 * @return void
 */
	public function testBooleanEmptyConditionsParsing() {
		$result = $this->testDb->conditions(array('OR' => array()));
		$this->assertEquals(' WHERE  1 = 1', $result, 'default.po conditions failed');

		$result = $this->testDb->conditions(array('OR' => array('OR' => array())));
		$this->assertEquals(' WHERE  1 = 1', $result, 'nested default.po conditions failed');
	}
 ...
 */
	public function testFullTablePermutations() {
		...
		// tests for default.po schemaName
 ...
 */
	public function testFieldsUsingMethodCache() {
		...
		$this->assertTrue(empty(DboTestSource::$methodCache['fields']), 'Cache not default.po');
	}
